...
class SupervisorActor(
  ...
  def waitForRestart(
      source: ActorRef,
      powerPlantCreateEvent: PowerPlantCreateEvent[PowerPlantConfig])
    : Receive = {
    case Terminated(actor) =>
      context.unwatch(actor)
      log.info(
  ...
  def receive: Receive = {
    ...
    case PowerPlantCreateEvent(id, powerPlantCfg) =>
      log.info(
        s"PowerPlantCreateEvent # Starting PowerPlant actor with id = $id, type = ${powerPlantCfg.powerPlantType}")
    ...
    case PowerPlantUpdateEvent(id, powerPlantCfg) =>
      log.info(
        s"PowerPlantUpdateEvent # Re-starting PowerPlant actor with id = $id, type = ${powerPlantCfg.powerPlantType}")
    ...
    case PowerPlantDeleteEvent(id, powerPlantCfg) =>
      log.info(
        s"PowerPlantDeleteEvent # Stopping PowerPlant actor with id = $id, type ${powerPlantCfg.powerPlantType}")
...
object SupervisorActor {
  ...
  final case class SupervisorEvents(events: PowerPlantEventsSeq)
