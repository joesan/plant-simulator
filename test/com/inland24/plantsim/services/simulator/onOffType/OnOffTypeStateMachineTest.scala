  ...
  Feature("PowerPlantState # init") {
    Scenario(
      ...
      initState.signals.foreach {
        case (key, value) if key == StateMachine.powerPlantIdSignalKey =>
          assert(value === onOffTpeCfg.id.toString)
        case (key, value) if key == StateMachine.activePowerSignalKey =>
          assert(value === onOffTpeCfg.minPower.toString)
        case (key, value) if key == StateMachine.isAvailableSignalKey =>
          assert(value.toBoolean)
        case (key, value) if key == StateMachine.isOnOffSignalKey =>
          assert(!value.toBoolean) // should be Off when initializing
      }
