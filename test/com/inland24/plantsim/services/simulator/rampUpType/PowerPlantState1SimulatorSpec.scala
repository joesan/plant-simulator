...
import com.inland24.plantsim.services.simulator.rampUpType.PowerPlantState1.{activePowerSignalKey, isAvailableSignalKey, isDispatchedSignalKey}
...
class PowerPlantState1SimulatorSpec extends FlatSpec {
  ...
  "PowerPlantState#empty" should "start with a default state" in {
    val emptyState = PowerPlantState1.empty(cfg.id, cfg.minPower, cfg.maxPower, cfg.rampPowerRate, cfg.rampRateInSeconds)
  ...
  "PowerPlantState#init" should "initialize the default signals " +
    ...
    initState.signals.foreach {
      case (key, value) if key == PowerPlantState1.isDispatchedSignalKey      => assert(!value.toBoolean)
      case (key, value) if key == PowerPlantState1.isAvailableSignalKey => assert(value.toBoolean)
      case (key, value) if key == PowerPlantState1.activePowerSignalKey => assert(value.toDouble === cfg.minPower)
    }
