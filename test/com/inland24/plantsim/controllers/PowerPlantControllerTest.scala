...
class PowerPlantControllerTest
  ...
  Feature("PowerPlantController ## createPowerPlant") {
    ...
    Scenario("not create a new PowerPlant for an Unknown PowerPlantType Config") {
      ...
      val expected =
        """{"message":"Invalid PowerPlantConfig List((/powerPlantType,List(JsonValidationError(List(Invalid PowerPlantType UnknownType. Should be one of RampUpType or OnOffType),List()))))"}""".stripMargin
